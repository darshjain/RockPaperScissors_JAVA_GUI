...
class mGame {
	...
	public static void introductionPanel() { // give the instruction to the game
		...
		JOptionPane.showMessageDialog(null, text, "How to play!", 0,
				new ImageIcon(System.getProperty("user.dir") + "/image/5.gif"));
	}
	...
	public static void gamePanel() { // the main game panel
		...
		Container panel = frame.getContentPane(); // creating a container panel, so we can place buttons where we
													// pleased
		panel.setLayout(null);

		String[] iconString = new String[3]; // creating icon string name so we can place the directory in with little
												// effort
		...
		b1.addActionListener( // next three button will listen for which play pick and calculate the win in
								// computeWinner
	...
	public static void computeWinner(int x) { // computing the winner
		...
		JLabel l4 = new JLabel(
				new ImageIcon(System.getProperty("user.dir") + "/image/" + (computerChoice - 1) + ".jpg"));
		...
		JLabel l6 = new JLabel("Tie: " + score_tie);
